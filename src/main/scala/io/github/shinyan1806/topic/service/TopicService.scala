...
import io.github.shinyan1806.topic.model.RepositoryTopic
...
trait TopicService {
  ...
  def getTopics(userName: String, repositoryName: String)(implicit session: Session): Seq[Topic] =
    (repositoryTopics.filter(
      rt => rt.userName === userName.bind && rt.repositoryName === repositoryName.bind,
  ...
  def searchTopics(query: String)(implicit session: Session): Seq[Topic] =
    topics
      .filter(_.name like s"%${query.toLowerCase}%")
      .sortBy(_.name)
      .list

  def getRepositoryNamesByTopic(
      topicName: String,
    ...
    (repositoryTopics join topics on (_.topicId === _.topicId))
      ...
      .map(
        x => (x._1.userName, x._1.repositoryName),
      )
      .list

  def addTopic(userName: String, repositoryName: String, topicName: String)(implicit
      session: Session,
  ): Unit = {
    val topicId: Int =
      topics
        .filter(_.name === topicName.bind)
        .map(_.topicId)
        .firstOption
        .getOrElse {
  ...
  def removeTopic(userName: String, repositoryName: String, topicName: String)(implicit
      session: Session,
  ): Unit =
    topics.filter(_.name === topicName.bind).map(_.topicId).firstOption.foreach {
      topicId =>
        repositoryTopics
          .filter(
            rt =>
